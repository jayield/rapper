...
public class DataMapper<T extends DomainObject<K>, K> implements Mapper<T, K> {
    ...
    private final Set<String> dmlCommands = new HashSet<String>() {
        {
    ...
    @Override
    public CompletableFuture<Integer> create(T obj) {
        ...
        int[] rows = { 1 };
        getParentMapper().ifPresent(objectDataMapper -> rows[0] = objectDataMapper.create(obj).join());
        //If the id is autogenerated, it will be set on the obj by the insert of the parent

        if(rows[0] == 0) return CompletableFuture.completedFuture(rows[0]);
        ...
        return SQLUtils.execute(mapperSettings.getInsertQuery(), stmt ->
            setValuesInStatement(
                ...
                .thenApply(ps ->{
                    int s = getNumberOfRows(ps);
                    func.wrap().accept(ps);
                    return rows[0] + getNumberOfRows(ps);
                })
    ...
    private int getNumberOfRows(PreparedStatement ps) {
        try {
            int updateCount = ps.getUpdateCount();
            for (String dml : dmlCommands) {
                if (mapperSettings.getInsertQuery().toUpperCase().startsWith(dml) && updateCount == 0) {
                    updateCount = -1;
                    break;
                }
            }
    ...
    public CompletableFuture<Integer> update(T obj) {
        ...
        int[] rows = { 1 };
        getParentMapper().ifPresent(objectDataMapper -> rows[0] = objectDataMapper.update(obj).join());
    ...
    @Override
    public CompletableFuture<Integer> delete(T obj) {
        ...
        return SQLUtils.execute(mapperSettings.getDeleteQuery(), stmt ->
                ...
                .thenCompose(preparedStatement -> {
                    ...
                    return getParentMapper()
                            .map(
                                    objectDataMapper -> objectDataMapper.delete(obj)
                                            .thenApply(rows -> getNumberOfRows(preparedStatement) + rows)
                            )
                            .orElse(CompletableFuture.completedFuture(getNumberOfRows(preparedStatement)));
                })
                .exceptionally(throwable -> {
